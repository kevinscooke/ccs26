...
function normalizeEvents(v2events: EventV2[], venues: VenueV2[]) {
  ...
  return v2events.map((e: any) => {
    if (!e || typeof e !== 'object') {
      return e; // Return as-is if invalid
    }
    ...
    try {
      ...
      return {
        ...
        isRecurring: false,
        socialLinks: [],
        city: venue && venue.slug ? { id: '', slug: venue.slug, name: venue.city || '', region: venue.state || '' } : null,
        venue: venue && venue.slug
          ? {
              id: venue.id,
              name: venue.name,
      ...
      // If normalization fails, return event as-is (graceful degradation)
      return e;
    }
  });
}
...
export async function loadEvents(): Promise<any[]> {
  try {
    ...
    // If normalization fails, return events as-is (graceful degradation)
    try {
      const venues = await loadVenues();
      return normalizeEvents(events as EventV2[], venues);
    } catch (normalizeErr) {
      // If venue loading/normalization fails, return events without normalization
      // This prevents build failures if venues.json is missing or malformed
      console.warn('Failed to normalize events with venue slugs:', normalizeErr);
      return events;
    }
  } catch (err) {
    // If events.json can't be loaded, return empty array
